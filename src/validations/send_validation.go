...
// validateAccountID validates that the account ID is provided and has valid format
func validateAccountID(accountID string) error {
	if accountID == "" {
		return pkgError.ValidationError("account_id: cannot be blank")
	}
	if len(accountID) < 1 || len(accountID) > 50 {
		return pkgError.ValidationError("account_id: must be between 1 and 50 characters")
	}
	return nil
}
...
func ValidateSendMessage(ctx context.Context, request domainSend.MessageRequest) error {
	...
	// Custom validation for account ID
